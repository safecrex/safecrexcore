...
const char * const BITCOIN_CONF_FILENAME = "safecrex.conf";
const char * const BITCOIN_PID_FILENAME = "safecrexd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "safecrex" is a composite category enabling all Safecrex-related debug output
            if(ptrCategory->count(string("safecrex"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SafecrexCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SafecrexCore
    // Mac: ~/Library/Application Support/SafecrexCore
    // Unix: ~/.safecrexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SafecrexCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SafecrexCore";
#else
    // Unix
    return pathRet / ".safecrexcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty safecrex.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override safecrex.conf
