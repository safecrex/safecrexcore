...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "SafecrexNetwork") type = SafecrexNetwork;
    if(strType == "SafecrexNetworkVariable") type = SafecrexNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == SafecrexNetwork) s = "SafecrexNetwork";
    if(type == SafecrexNetworkVariable) s = "SafecrexNetworkVariable";
