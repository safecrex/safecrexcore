 ...
 *
 *   // network
 *   CSafecrexNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.safecrex.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CSafecrexNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CSafecrexNetwork(UniValue objIn)
    {
...
// // can be under: SafecrexNetwork
// //   -- signature requirements : Key1(User)
// class CSafecrexNetworkVariable : public CGovernanceObject
// {
// private:
